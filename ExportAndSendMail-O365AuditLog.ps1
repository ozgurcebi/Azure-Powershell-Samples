Function Split-O365AuditLogs-FromO365 ()
{
	...
	$username = "ozgur@microsoftdemocenter.com"
	$Password = ConvertTo-SecureString "Netas2016" -AsPlainText -Force
	...
	$endDate = Get-Date
	$SpecifiedUserIDs = "emre.martin@microsoftdemocenter.com", "ertan@microsoftdemocenter.com" #syntax: "<value1>","<value2>",..."<valueX>".
	...
	$fileName = ("D:\AuditLog\CSVExport\" + $datestring + ".csv")
    
	Write-Host (" >>> writing to file {0}" -f $fileName)
	$data | Export-csv $fileName -NoTypeInformation

    $SmtpCred = $UserCredential
    $ToAddress = 'ocebi@netas.com.tr'
    $FromAddress = 'ozgur@microsoftdemocenter.com'
    ...
    $options = @{
     ...
     From = $FromAddress
     Subject = "AuditLogs" 
     Body = "This mail contains AuditLogs"
